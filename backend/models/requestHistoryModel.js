...
// ✅ ดึงรายการคำขอทั้งหมดของ user (หรือทั้งหมดถ้าเป็น staff)
exports.getAllRequests = async (userId = null) => {
  let query = `
    SELECT 
      r.request_id,
      r.request_code,
      r.request_date,
      r.request_status,
      r.request_type,          -- ประเภท (เบิก / ยืม)
      r.is_urgent,             -- เร่งด่วนหรือไม่
      r.request_note,
      r.updated_at,
      u.user_fname || ' ' || u.user_lname AS user_name,
      COUNT(rd.request_detail_id) AS total_items,   -- ✅ จำนวนรายการย่อย
      COALESCE(SUM(rd.requested_qty),0) AS total_requested, -- ✅ รวมจำนวนที่ขอ
      COALESCE(SUM(rd.approved_qty),0) AS total_approved    -- ✅ รวมจำนวนที่อนุมัติ
    FROM requests r
...
exports.getRequestById = async (requestId) => {
  ...
  const headerQuery = `
    ...
    GROUP BY r.request_id, u.user_fname, u.user_lname
  `;
  const headerRes = await pool.query(headerQuery, [requestId]);
  ...
  const detailQuery = `
    SELECT
      ...
      COALESCE(med.med_code, ms.medsup_code, eq.equip_code, md.meddevice_code, gen.gen_code) AS item_code
