...
exports.getExpiredReport = async (filters) => {
    ...
    // เงื่อนไขการกรองใน WHERE clause
    ...
    // เงื่อนไขการกรองใน HAVING clause (ใช้สำหรับ aggregate functions)
    ...
    // เงื่อนไขการกรองใน WHERE clause
    console.log('Filtering by date range:', { start_date, end_date });
    if (start_date && end_date) {
        params.push(start_date, end_date);
        whereConditions.push(`l.exp_date BETWEEN $${params.length - 1} AND $${params.length}`);
    }

    // สร้าง WHERE และ HAVING clause จากเงื่อนไขทั้งหมด
    ...
    const query = `
      ...
      ${whereClause}
      GROUP BY i.item_name, i.item_category, l.lot_no, l.exp_date, l.qty_imported, e.expired_qty
      ${havingClause}
      ORDER BY l.exp_date ASC;
    `;
    
    console.log('Final Expired Report Query:', query);
