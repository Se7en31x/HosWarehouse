...
exports.getAllExpired = async () => {
  const result = await pool.query(
    `SELECT 
        ...
        l.lot_no,
        l.exp_date,
        l.qty_imported,   -- ✅ เพิ่มจำนวนรับเข้ามา
        u.user_name AS reported_by, -- ผู้รายงาน
     ...
     LEFT JOIN LATERAL (   -- ✅ subquery หาผู้ทำลายล่าสุด
        SELECT 
          a.action_by,
          usr.user_fname || ' ' || usr.user_lname AS action_by_name
        FROM expired_actions a
        LEFT JOIN users usr ON a.action_by = usr.user_id
     ...
     GROUP BY ei.expired_id, ei.expired_date, ei.expired_qty, ei.note,
              ...
              u.user_name, last_action.action_by_name
