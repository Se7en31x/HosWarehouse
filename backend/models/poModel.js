const { pool } = require('../config/db'); // ✅ ใช้ pool ที่ตั้งไว้
const qn = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0);

// ────────────────────────────────────────────────
// ✅ Helper: สร้างเลขที่ PO (Running ตามเดือน)
// ────────────────────────────────────────────────
async function generatePoNo(client) {
    const now = new Date();
    ...
    let running = 1;
    if (rows.length > 0) {
        const lastPoNo = rows[0].po_no;          // เช่น PO-202508-0012
        const lastRun = parseInt(lastPoNo.split('-')[2], 10);
        if (!isNaN(lastRun)) running = lastRun + 1;
    }
...
exports.getById = async (poId) => {
    ...
    try {
        const head = await client.query(
            `SELECT p.*, 
              s.supplier_name, s.supplier_address, s.supplier_contact_name, 
              s.supplier_phone, s.supplier_email
         FROM purchase_orders p
         LEFT JOIN suppliers s ON s.supplier_id = p.supplier_id
...
exports.createIssued = async ({ payload, userId }) => {
    ...
    try {
        ...
        if (!supplierId && payload.vendor_manual?.name) {
         ...
         (supplier_name, supplier_address, supplier_contact_name, supplier_phone, supplier_email, supplier_note)
        ...
        // 3) gen po no
        const poNo = await generatePoNo(client);
...
// ❌ ยกเลิก PO
// ────────────────────────────────────────────────
exports.cancel = async ({ poId, reason }) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const head = await client.query(
            `SELECT po_status FROM purchase_orders WHERE po_id = $1 FOR UPDATE`,
            [poId]
        );
        if (head.rowCount === 0) throw new Error('PO not found');
        if (head.rows[0].po_status === 'canceled') {
            const err = new Error('PO already canceled');
            err.code = 'PO_ALREADY_CANCELED';
            throw err;
        }
        await client.query(
...
// ───────────────────────────────
// ดึง PR ด้วย pr_id (เลข int)
// ───────────────────────────────
exports.getPurchaseRequestById = async (prId) => {
    const client = await pool.connect();
    try {
        const headerRes = await client.query(
            `SELECT pr_id, pr_no, request_date, requester_id, requester_name, status
             FROM pr_header
             WHERE pr_id = $1`,  // <-- pr_id ต้องเป็น int
            [prId]
        );
       ...
       i.item_unit,
       i.item_zone,
       COALESCE(m.med_code, s.sup_code, e.eq_code, md.device_code, g.gen_code) AS item_code
FROM pr_detail d
JOIN items i ON d.item_id = i.item_id
LEFT JOIN medicine_detail m ON i.item_id = m.item_id
LEFT JOIN medsup_detail s ON i.item_id = s.item_id
...
// ───────────────────────────────
// ดึง PR ด้วย pr_no (string เช่น PR-202508-2675)
// ───────────────────────────────
exports.getPurchaseRequestByNo = async (prNo) => {
    const client = await pool.connect();
    try {
        // header
        const headerRes = await client.query(
            `SELECT ph.pr_id,
                    ...
                    ph.reason
             FROM pr_header ph
             LEFT JOIN users u ON ph.requester_id = u.user_id
             WHERE ph.pr_no = $1`,
            [prNo]
        );
        ...
        return { header, items: detailRes.rows };
    } finally {
        client.release();
    }
};
...
exports.listFiles = async (poId) => {
    const { rows } = await pool.query(
       ...
       FROM po_files WHERE po_id = $1 ORDER BY uploaded_at DESC`,
        [poId]
    );
    return rows;
};
...
exports.deleteFile = async ({ poId, fileId }) => {
    await pool.query(
        `DELETE FROM po_files WHERE file_id=$1 AND po_id=$2`,
        [fileId, poId]
    );
};
